   ...
   public class PlayerManager extends EventDispatcher
   {
      ...
      private function __itemEquip(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(_loc5_ != null)
         {
			...
			// begin ghost item star
			var _totalGhostPlace:int = 0;
			if((_totalGhostPlace = _loc2_.readInt()) > 0)
			{
				var _loc22_:EquipGhostData = null;
				var _loc24_:int = 0;
				var _loc12_:int = 0;
				var _loc39_:int = 0;
				while(_loc39_ < _totalGhostPlace)
				{
					_loc24_ = _loc2_.readInt();
					_loc12_ = _loc2_.readInt();
					(_loc22_ = new EquipGhostData(_loc24_,_loc12_)).level = _loc2_.readInt();
					_loc22_.totalGhost = _loc2_.readInt();
					_loc5_.addGhostData(_loc22_);
					_loc39_++;
				}
			}
			// end ghost item star
