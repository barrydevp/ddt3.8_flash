package ddt.utils
{
   ...
   public class StaticFormula
   {
	  ...
	  public static function getPropertyAddition(param1:int, param2:int) : Number
	  {
		  var _loc3_:Number = param1 * Math.pow(1.1,param2) - param1;
		  return Math.round(_loc3_);
	  }
      ...
      public static function getDamage(param1:PlayerInfo) : int
      {
         ...
         if(_loc3_)
         {
			 ...
			 var ghostProperty:GhostPropertyData = getGhostPropertyData(_loc3_);
			 if(ghostProperty)
			 {
				 _loc2_ += ghostProperty.mainProperty;
			 }
         }
	  ...
	  private static function getGhostPropertyData(param1:InventoryItemInfo) : GhostPropertyData
	  {
		  ...
		  var _loc4_:PlayerInfo;
		  var _loc6_:* = (_loc4_ = PlayerInfoViewControl.currentPlayer || PlayerManager.Instance.Self).ID != PlayerManager.Instance.Self.ID;
		  var _loc2_:Boolean = _loc3_.BagType == 0 && _loc3_.Place <= 30;
		  return !!_loc2_ ? EquipGhostManager.getInstance().getPorpertyData(_loc3_,_loc4_) : null;
	  }
