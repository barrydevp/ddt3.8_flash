package ddt.data.goods
{
   ...
   public class InventoryItemInfo extends ItemTemplateInfo
   {
	  ...
	  public function getTotalStrengthenLevel(): int {
		  return !!this.isGold ? (this.StrengthenLevel + 1) : this.StrengthenLevel;
	  }
	  
	  public function getAdditionalPropertyByStrenLvl(property:String): uint {
		  var lvl:int = this.getTotalStrengthenLevel();
		  if(lvl <= 0){
			  return 0;
		  }
		  ...
		  switch(property) {
			  case "hert":
				  return StaticFormula.getHertAddition(int(this.Property7), lvl);
			  case "armor":
				  return StaticFormula.getDefenseAddition(int(this.Property7), lvl);
			  case "recoverhp":
				  return StaticFormula.getRecoverHPAddition(int(this.Property7), lvl);
		  }
		  
		  return 0;
	  }
	  
	  public function getAdditionalPropertyByStarLvl(property:String, starLevel: uint): uint {
		  if(starLevel <= 0){
			  return 0;
		  }
		  ...
		  switch(property) {
			  case "hert":
				  return StaticFormula.getHertAddition(int(this.Property7), starLevel);
			  case "armor":
				  return StaticFormula.getDefenseAddition(int(this.Property7), starLevel);
			  case "recoverhp":
				  return StaticFormula.getRecoverHPAddition(int(this.Property7), starLevel);
			  case "attack":
				  return StaticFormula.getPropertyAddition(int(this.Attack), starLevel);
			  case "defense":
				  return StaticFormula.getPropertyAddition(int(this.Defence), starLevel);
			  case "agility":
				  return StaticFormula.getPropertyAddition(int(this.Agility), starLevel);
			  case "luck":
				  return StaticFormula.getPropertyAddition(int(this.Luck), starLevel); 
		  }
